...
```C:\Program Files (x86)\Steam\userdata\85588318\631510\remote\dmc1.sav```
...
If you have used DDMK before, chances are your game is already properly downgraded. 

Note: The mod will not load hooks for DDMK if it is not version 2.7.3 from Github. Additionally, while the mod will not stop itself from loading if DMC1 is not properly patched. Issues or crashes will most likely occur.
## 2. Install the randomizer.
If you already have a `dinput8.dll` from another mod, rename it to something else and extract the randomizers `dinput8.dll` into the same directory as `dmc1.exe` 
...
If the mod has been successfully installed, when start up DMC3 and you should have a console window and potentially a modified DDMK overlay. 

## 3. Connect to a room
It is recommended that before launching DMC1, you start up the DMC3 Client available in the Archipelago launcher. This launcher is what you use to connect to the room.

Once the client is connected, start up Devil May Cry 1, it should then connect to the Archipelago client. You can use the command "/dmc3" in the Archipelago client to verify that it is connected
...
There is a `steam_appid.txt` file in the zip, put this in the same place as your `dmc1.exe`, this will allow you to start up DMC3 from its EXE and skip the launcher. While technically this should be optional, people have reported that the mod acts up without the file.

This also works for the same for DMC3 and 2, DMC2 however will default to Dante unless launched with `lucia` as an argument.
