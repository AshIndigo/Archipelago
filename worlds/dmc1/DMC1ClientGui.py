...
from .DMC1Client import DMC1Context
...
class DMC3Manager(GameManager):
    ...
    base_title = "Archipelago Devil May Cry 1 Client"
...
def start_gui(ctx: DMC1Context):
    ctx.ui = DMC3Manager(ctx)
    ctx.ui_task = asyncio.create_task(ctx.ui.async_run(), name="UI")
    import pkgutil

    data = pkgutil.get_data(DevilMayCry1World.__module__, "dmc1client.kv").decode()
