...
class DMC1CommandProcessor(ClientCommandProcessor):
    def _cmd_dmc1(self):
        """Check DMC1 Connection State"""
        if isinstance(self.ctx, DMC1Context):
            if not self.ctx.is_proxy_connected():
                logger.info(f"DMC1 Status: Not connected")
            else:
                logger.info(f"DMC1 Status: Connected")
...
class DMC1Context(CommonContext):
    ...
    async def server_auth(self, password_requested: bool = False):
        if password_requested and not self.password:
            await super(DMC1Context, self).server_auth(password_requested)
    ...
    def on_package(self, cmd: str, args: dict):
        match cmd:
            case "Connected":
                if DEBUG:
                    print(args)
                from . import DevilMayCry1World
                args["slot_data"]["client_version"] =  DevilMayCry1World.world_version
...
async def proxy(websocket, path: str = "/", ctx: DMC1Context = None):
...
async def on_client_connected(ctx: DMC1Context):
...
async def proxy_loop(ctx: DMC1Context):
    try:
        ...
        logger.exception(e)
        logger.info("Aborting DMC1 Proxy Client due to errors")
...
def launch(*launch_args: str):
    async def main():
        ...
        ctx = DMC1Context(args.connect, args.password)
        logger.info("Starting DMC1 Proxy Client")
        ctx.proxy = websockets.serve(functools.partial(proxy, ctx=ctx),
                                     host="localhost", port=82301, ping_timeout=999999, ping_interval=999999)
        ctx.proxy_task = asyncio.create_task(proxy_loop(ctx), name="ProxyLoop")

        if gui_enabled:
            from .DMC1ClientGui import start_gui
            start_gui(ctx)
        ctx.run_cli()
